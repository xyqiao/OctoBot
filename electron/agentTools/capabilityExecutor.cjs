...
async function taskCreateDefinition(args = {}) {
  ...
  const inferredText = toSafeString(payload.text || payload.query || payload.content || payload.prompt, "").trim();
  const prompt = toSafeString(payload.prompt, "").trim();

  const finalTitle = title || (inferredText ? inferredText.split(/\n|。|！|!|\?|？/)[0].slice(0, 28).trim() : "自动任务");
